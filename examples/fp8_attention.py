"""FP8 Attention kernel - Fixed to match Triton's algorithm."""
...
def prepare_fp8_attention_inputs(
    ...
    Returns:
        q_reshaped: [batch*heads, seq, dim]
        k_reshaped: [batch*heads, seq, dim]
        v_transposed: [batch*heads, dim, seq]
    ...
    return q_reshaped, k_reshaped, v_transposed, (batch, heads, seq_len, head_dim)
...
def fp8_attention_tritonbench(
    ...
    q_reshaped, k_reshaped, v_reshaped, shape = prepare_fp8_attention_inputs(q, k, v)
    batch, heads, seq_len, head_dim = shape

    # Clamp values before converting to FP8 to avoid overflow to inf
    q_clamped = q_reshaped.clamp(FP8_E5M2_MIN, FP8_E5M2_MAX)
    k_clamped = k_reshaped.clamp(FP8_E5M2_MIN, FP8_E5M2_MAX)
    v_clamped = v_reshaped.clamp(FP8_E5M2_MIN, FP8_E5M2_MAX)

    # Convert to FP8 - use e5m2 to match triton_flash_v2
    q_fp8 = q_clamped.to(torch.float8_e5m2)
    k_fp8 = k_clamped.to(torch.float8_e5m2)
    v_fp8 = v_clamped.to(torch.float8_e5m2)
...
# FP8 E5M2 constants
FP8_E5M2_MAX = 57344.0  # max finite value E5M2 can represent
...
def fp8_attention_pytorch(
    ...
    """
    # Common preprocessing
    q_reshaped, k_reshaped, v_transposed, shape = prepare_fp8_attention_inputs(q, k, v)
    batch, heads, seq_len, head_dim = shape

    sm_scale = 1.0 / math.sqrt(float(head_dim))

    # No dimension requirements since we're using regular matmul after dequantization

    outputs = []

    for i in range(batch * heads):
        q_i = q_reshaped[i]  # [seq, dim]
        k_i = k_reshaped[i]  # [seq, dim]
        v_i = v_transposed[i]  # [dim, seq] - pre-transposed

        # Clamp values before converting to e5m2 to avoid overflow
        q_i_clamped = q_i.clamp(FP8_E5M2_MIN, FP8_E5M2_MAX)
        k_i_clamped = k_i.clamp(FP8_E5M2_MIN, FP8_E5M2_MAX)

        # Convert to e5m2
        q_fp8 = q_i_clamped.to(torch.float8_e5m2)  # row-major [seq, dim]
        k_fp8 = k_i_clamped.to(torch.float8_e5m2)  # row-major [seq, dim]

        # For Q @ K^T, we need K^T to be column-major
        kt_fp8 = k_fp8.t()  # column-major [dim, seq]

        # Q @ K^T - dequantize and use regular matmul since e5m2 not supported by _scaled_mm
        q_deq = q_fp8.to(torch.float32)
        ...
        p_norm_clamped = p_norm.clamp(FP8_E5M2_MIN, FP8_E5M2_MAX)
        p_fp8 = p_norm_clamped.to(torch.float8_e5m2)  # row-major [seq, seq]

        # v_i is [dim, seq], need to make it row-major first
        v_i_contiguous = v_i.contiguous()  # row-major [dim, seq]
        v_i_clamped = v_i_contiguous.clamp(FP8_E5M2_MIN, FP8_E5M2_MAX)
        v_fp8 = v_i_clamped.to(torch.float8_e5m2)  # row-major [dim, seq]
        vt_fp8 = v_fp8.t()  # column-major [seq, dim]
...
if __name__ == "__main__":
    # Test the implementation - ensure dimensions divisible by 16
    torch.manual_seed(42)
    batch, heads, seq_len, head_dim = 1, 2, 128, 64  # 128 and 64 are divisible by 16
    ...
    ref = fp8_attention_pytorch(q, k, v)
